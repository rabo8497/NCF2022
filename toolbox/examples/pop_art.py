...
# https://github.com/aluscher/torchbeastpopart/blob/master/torchbeast/core/popart.py
...
class PopArtLayer(torch.nn.Module):
    ...
    def update_parameters(self, vs, task):
        ...
        self.weight.data = (self.weight.t() * old_sigma / self.sigma).t()
        self.bias.data = (old_sigma * self.bias + old_mu - self.mu) / self.sigma
